...
The function filters the 'notes' field associated with each missed game due to an injury
or Movement to/from injured/inactive list.

"""

def notes_filter(c):
    # conver string to lower case characters
    note = c['Notes']
    ...
    # if no reason has been given for a missed game or move to inactive list, assume it was a non-injury move
    elif lower_case_note == 'placed on il' or lower_case_note == 'placed on il (p)':
        return 'roaster move', 'healthy inactive'
    ...
    #-------Sick Inactive  -------------------------------
    elif any (x in lower_case_note for x in ['virus', 'headace', 'flu', 'sick', 'illness','infection','pneumonia', 'gastro','appende','nausea', 'pox', 'dizziness', 'poisoning','bronchitis']):
        return 'sick', 'sick'
